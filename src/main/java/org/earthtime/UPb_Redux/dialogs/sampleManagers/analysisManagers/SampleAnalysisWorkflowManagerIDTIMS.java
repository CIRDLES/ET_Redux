 ...
 */
public class SampleAnalysisWorkflowManagerIDTIMS extends DialogEditor implements SampleAnalysisWorkflowManagerInterface {
    ...
    private JComboBox<String> masterTracerChooser = null;
    ...
    private JComboBox<String> masterPbBlankChooser = null;
    private JButton masterPbBlankFiller = null;
    private JComboBox<String> masterInitialPbModelChooser = null;
    ...
    private ArrayList<JComboBox<String>> fractionInitialPbChoice;
    ...
    public void initAliquot(final AliquotInterface aliquot) {
        ...
        for (int i = (mtracers.size() > 1 ? 1 : 0); i < mtracers.size(); i++) {
            masterTracerChooser.addItem(mtracers.get(i).getNameAndVersion());
        }
...
//            }
    ...
    private void addNewFractionRow(AliquotInterface aliquot, ETFractionInterface fraction) {
    ...
    private void buildFastEditDisplayPanel() {
        ...
        for (int f = 0; f
            ...
            myHorizFraction.add(jPanel2Layout.createSequentialGroup()//
                    ...
                    .add(4, 4, 4)//
                    .add(fractionID.get(f), 100, 100, 100) //
                    .add(0, 0, 0)//
...
//            }
    ...
    private void updateFractionRow(ETFractionInterface tempFrac, int row) {
        ...
        ((AbstractButton) fractionZirconCheckBox.get(row)).setSelected(((FractionI) tempFrac).isZircon());
    ...
    private void initSampleFields() throws ETException {
        ...
        sampleIGSN_text.setDocument(
                new UnDoAbleDocument(sampleIGSN_text, true));// per doug walker oct 2014 !mySample.isArchivedInRegistry()));
        ...
        sampleRegistryChooser.setEnabled(true);//per doug walker oct 2014!mySample.isArchivedInRegistry());
    ...
    private void saveAliquotFraction(AliquotInterface aliquot, ETFractionInterface tempFrac)
        ...
        if (((FractionI) tempFrac).isZircon() != ((JCheckBox) fractionZirconCheckBox.get(row)).isSelected()) {
            ((FractionI) tempFrac).setZircon(((AbstractButton) fractionZirconCheckBox.get(row)).isSelected());
...
//                ex.printStackTrace();
        ...
        try {
            ...
            ((FractionI) tempFrac).setInitialPbModel(initialPbModel);

            ((FractionI) tempFrac).calculateStaceyKramersInitialPbModelValues();
...
//                }
                    ...
                    Vector<ETFractionInterface> fractions = ((UPbReduxAliquot) mySample.getAliquots().get(i)).getAliquotFractions();
    ...
    private void initComponents() {
        ...
        sampleType_label.setText("ID-TIMS Workflow Manager for   A N A L Y S I S     M O D E    of a Sample");
        ...
        fractionTableHeader.setForeground(new java.awt.Color(204, 51, 0));
        fractionTableHeader.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        fractionTableHeader.setText("                            Fraction ID                                                Tracer                     Tracer Mass     Fraction Mass                    Pb Blank                         Initial Pb Model                    Est Date         Pb Blank Mass");
        fractionTableHeader.setOpaque(true);
        jLayeredPane1.add(fractionTableHeader);
        fractionTableHeader.setBounds(20, 280, 1080, 20);
    ...
    private javax.swing.JLabel fractionTableHeader;
