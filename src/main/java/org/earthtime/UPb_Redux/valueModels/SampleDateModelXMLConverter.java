/*
 * SampleDateModelXMLConverter.java
 *
 * Created on February 5, 2008, 6:21 PM
 *
 *
 ...
 * between <code>SampleDateModels</code> and XML files.
 * 
 ...
 */
public class SampleDateModelXMLConverter implements Converter {
    ...
    public boolean canConvert ( Class clazz ) {
        return clazz.equals( SampleDateModel.class );
    }
    ...
    public void marshal ( Object value, HierarchicalStreamWriter writer,
        ...
        ValueModel dateModel = (SampleDateModel) value;
        ...
        writer.setValue( ((SampleDateModel) dateModel).getMeanSquaredWeightedDeviation().toPlainString() );
        writer.endNode();

        writer.startNode( "internalTwoSigmaUnct" );
        writer.setValue( ((SampleDateModel) dateModel).getInternalTwoSigmaUnct().toPlainString() );
        writer.endNode();

        writer.startNode( "internalTwoSigmaUnctWithTracerCalibrationUnct" );
        writer.setValue( ((SampleDateModel) dateModel).getInternalTwoSigmaUnctWithTracerCalibrationUnct().toPlainString() );
        writer.endNode();

        writer.startNode( "internalTwoSigmaUnctWithTracerCalibrationAndDecayConstantUnct" );
        writer.setValue( ((SampleDateModel) dateModel).getInternalTwoSigmaUnctWithTracerCalibrationAndDecayConstantUnct().toPlainString() );
        writer.endNode();

        writer.startNode( "includedFractionsVector" );
        Vector<String> includedFractions = ((SampleDateModel) dateModel).getIncludedFractionIDsVector();
        for (String id : includedFractions) {
            writer.startNode( "fractionID" );
            writer.setValue( id );
            writer.endNode();
        }
        //context.convertAnother(((SampleDateModel) dateModel).getIncludedFractionIDsVector());
        //writer.setValue(((SampleDateModel)dateModel).getIncludedFractionIDsVector());
        writer.endNode();

        writer.startNode( "explanation" );
        writer.setValue( ((SampleDateModel) dateModel).getExplanation() );
        writer.endNode();

        writer.startNode( "comment" );
        writer.setValue( ((SampleDateModel) dateModel).getComment() );
        writer.endNode();

        writer.startNode( "preferred" );
        writer.setValue( Boolean.toString( ((SampleDateModel) dateModel).isPreferred() ) );
        writer.endNode();
